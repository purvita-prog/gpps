  ...
  public function build() {
    ...
    try {
      if (!empty($block_settings['override'])) {
        // When block override is enabled, use the block's page_name as well.
        $facebook = $this->facebook->createInstance(
          $block_settings['app_id'],
          $block_settings['secret_key'],
          $block_settings['user_token'],
          $block_settings['page_name']
        );
      }
      else {
        $facebook = $this->facebook->createInstance(
      ...
      // Prefer stored numeric page_id when available to avoid API lookups.
      $identifier = $this->config->get('page_id');
      if (empty($identifier)) {
        $identifier = $this->getSetting('page_name');
      }
      ...
      foreach ($posts as $post) {
        // Some posts may not include a status_type; provide a safe default and
        // still render using the base template as a fallback.
        $type = (isset($post['status_type']) && $post['status_type'] !== NULL && $post['status_type'] !== '') ? $post['status_type'] : 'default';
        $items[] = [
          '#theme' => [
            'socialfeed_facebook_post__' . $type,
