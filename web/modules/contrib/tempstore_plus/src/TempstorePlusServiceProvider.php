...
class TempstorePlusServiceProvider extends ServiceProviderBase {
   ...
   * {@inheritdoc}
   *
   * Note: This module's weight is set to 1 (see tempstore_plus_install()) to
   * ensure this alter() method runs after workspaces module's service provider.
   * This allows us to detect and extend workspaces' modifications to
   * layout_builder.tempstore_repository when both modules are enabled.
   */
  public function alter(ContainerBuilder $container) {
    ...
    if ($container->hasDefinition('layout_builder.tempstore_repository')) {
      ...
      // Only modify the service if:
      // 1. Our class is available
      // 2. We're not breaking the service (e.g., if workspaces will run after us)
      // Check if workspaces has already modified this service by checking the class.
      $currentClass = $definition->getClass();
      $isWorkspacesClass = ($currentClass === 'Drupal\workspaces\WorkspacesLayoutTempstoreRepository');

      // Only proceed if our class exists AND (workspaces already ran OR workspaces isn't installed).
      if (class_exists('Drupal\tempstore_plus\LayoutTempstoreRepository') &&
          ($isWorkspacesClass || !$container->hasDefinition('workspaces.manager'))) {
        $definition->setClass('Drupal\tempstore_plus\LayoutTempstoreRepository');
        $definition->setArguments([new Reference('tempstore_plus.strategy_selector')]);
        // Clear any method calls (e.g., setWorkspacesManager from workspaces module).
        // Workspace awareness is handled by WorkspaceKeyTrait in our strategies.
