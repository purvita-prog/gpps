...
class SitewideAlertManager {
  ...
  public function activeSitewideAlerts(): array {
    ...
    $sitewideAlertQuery = $sitewideAlertStorage
      ->getQuery()
      ->condition('status', 1)
      ->accessCheck();

    if (!$this->config->get('show_untranslated')) {
      $sitewideAlertQuery->condition('langcode', $sitewideAlertStorage->getEntityType()->isTranslatable() ? $langcode : LanguageInterface::LANGCODE_DEFAULT);
    }

    $sitewideAlertIds = $sitewideAlertQuery->execute();
