...
class SitewideAlertForm extends ContentEntityForm {
  ...
  public function buildForm(array $form, FormStateInterface $form_state): array {
    ...
    if (!empty($form['style']['widget']['#options'])) {
      ...
      elseif (!isset($form['style']['widget']['#default_value'])) {
        // When translating, the style field doesn't carry over from the source
        // language. Pre-populate it so translators don't have to re-select it.
        $value = $entity->getUntranslated()->get('style')->value;
        if ($value) {
          $form['style']['widget']['#default_value'] = $value;
        }
      }
    }
